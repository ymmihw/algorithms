...
public class SlopeOne {
  ...
  /**
   * Based on the available data, calculate the relationships between the items and number of
   * occurences
   * 
  ...
  /**
   * Based on existing data predict all missing ratings. If prediction is not possible, the value
   * will be equal to -1
   * 
   * @param data existing user data and their items' ratings
   */
  ...
  private static void print(HashMap<Item, Double> hashMap) {
    ...
    for (Item j : hashMap.keySet()) {
      System.out.println(
          " " + j.getItemName() + " --> " + formatter.format(hashMap.get(j).doubleValue()));
    }
  }
